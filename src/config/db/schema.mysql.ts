...
export const guestTrialQuota = table(
  ...
  {
    id: varchar191('id').primaryKey(),
    tokenHash: varchar191('token_hash').notNull().unique(),
    deviceHash: varchar191('device_hash').notNull(),
    ipHash: varchar191('ip_hash').notNull(),
    remainingCredits: int('remaining_credits').notNull().default(5),
    usedTaskCount: int('used_task_count').notNull().default(0),
