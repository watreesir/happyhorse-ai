...
2. 任务后续失败时会自动释放次数
   - 之前只有“提交上游前就失败”才会释放
   - 现在如果任务已经提交成功，但后续 query / notify 回来变成 `FAILED`，也会自动释放当天次数

3. `FREE_DAILY_VIDEO_LIMIT` 改为真正按数量生效
   - 之前实现实际上只支持 `1`
   - 现在如果以后改成 `2`、`3`，服务端会按真实次数上限判断

4. 不影响现有 SEO、价格页结构、Stripe 支付链路
   - 这次仅是服务端逻辑修正
   - 未引入新的数据库表结构变更
